...
class Settings(BaseSettings):
    ...
    # Накатывать миграции при старте. Годится там, где приложение живёт
    # постоянным процессом: своя машина, systemd, контейнер. На serverless
    # игнорируется — процесс там поднимается на каждый запрос, а не на выкатку.
    # По умолчанию выключено: изменение схемы необратимо, и включать его должен
    # тот, кто отвечает за базу.
