...
# Колонка удаляется в том же шаге, где добавляется новая. Поле прожило пару
# часов и только в разработке, боевой базы тогда не было — разносить перенос на
# две выкатки было не для кого. Для нового такого случая правило обратное:
# сначала добавить и перенести, удалить следующей выкаткой.
BREAKS_OLD_CODE = "поле прожило часы в разработке, боевой базы ещё не существовало"
