...
export const api = axios.create({
  ...
  // Тип содержимого намеренно не задан. Axios ставит application/json сам,
  // когда отправляет объект, — а вот при заданном заголовке он делает другое:
  // превращает форму с файлом в JSON и шлёт «{"file":{}}». Файл теряется, а
  // сервер отвечает 422 на пустое поле. Так ломались загрузка фотографии
